...
object LineOfSight {
  ...
  def lineOfSight(input: Array[Float], output: Array[Float]): Unit = {
    input.zipWithIndex.foreach { case (value, index) => {
      if (index == 0) output(index) = 0
      else output(index) = max(output(index-1), value/index)
    }}
  }
  ...
  def upsweepSequential(input: Array[Float], from: Int, until: Int): Float = {
    (from until until).foldLeft(0f) {(currMax, i) => max(currMax, input(i)/i)}
  }
  ...
  def downsweepSequential(input: Array[Float], output: Array[Float],
    ...
    var (i, maxVal) = (from, startingAngle)
    while(i < until) {
